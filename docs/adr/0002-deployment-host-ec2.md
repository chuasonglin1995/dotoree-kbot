...
- **Secrets:** one AWS Secrets Manager secret (`dotoree_kbot-prod`, JSON of all env vars) rendered into an `EnvironmentFile`. (Chosen over SSM Parameter Store: same flat namespace + clean `terraform destroy` teardown either way, but the team preferred a purpose-built secrets vault; ~$0.40/mo for the single JSON secret.)
